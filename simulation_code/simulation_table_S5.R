#This program can reproduce simulation results in Web Table S5 in Appendix
#that is, using IPCW weight in Wang et al.(2026)
...
for(case in case_set){
  ...
  while(i<=N){
    ...
    if(mint<0){ #to make sure generated death time and hospitalization time is >0
      ...
      #method 1
      res1 = pim.csd.m1(formula=y1~x,data=gdata,SurvTime="y1",Status="delta1",covariateC=c("z"),
                        priority = 1:2, L = L, adjusted = "unadjusted", model = "marginal",
                        link = "identity",method = "Newton",control = cntl)
      flag1[i] = res1$flag
      #method 2
      res2 = pim.csd.m1(formula=y1~x,data=gdata,SurvTime="y1",Status="delta1",covariateC=c("z"),
  ...
  res = round(resu,3)
  write.csv(res,paste0("tables5_choice_",choice,"_case_",case,"_n_",n,".csv"))
  cat("case=",case,"\n")
}
