ipcw.weight.m1 = function(tdata, trt_con, censoring_weight, win_status,
  ...
  for (q in seq_len(n_ep)) {
    ...
    # Keep the defining m1 feature: both components are evaluated using the
    # control-side comparison time / status logic, but now with the full
    # subject-specific K_i(t) rather than the incorrect diagonal extraction.
    surv_trt <- extract_surv(trt_id, con_index)
    ...
    G.i <- extract_G(trt_id, con_index)
