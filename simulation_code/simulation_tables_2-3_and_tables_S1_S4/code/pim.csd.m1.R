...
# Revised m1 implementation. The defining m1 weight logic is preserved,
# while the shared bugs (pair-order alignment and diagonal Kij extraction)
# are corrected.

pim.csd.m1 = function(formula, data, SurvTime, Status, covariateC = NULL, priority = 1:2,
  ...
  if (!(model %in% c("difference", "marginal"))) {
    stop("The revised pim.csd.m1() currently supports model='difference' or 'marginal'.")
  }
