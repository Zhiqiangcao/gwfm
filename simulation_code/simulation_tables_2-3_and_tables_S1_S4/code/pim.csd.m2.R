...
# Revised m2 implementation for the generalized win-fraction regression.
# The main corrections are:
#   1) pair design is built in the same order as merge.csd()/win.strategy();
#   2) restricted endpoints (y_n, delta_n) are kept as in the paper;
#   3) IPCW uses subject-specific K_i(t) instead of diag(Kij).
...
pim.csd.m2 = function(formula, data, SurvTime, Status, covariateC = NULL, priority = 1:2,
  ...
  # sort data by observed survival time of the fatal endpoint
  ...
  if (!(model %in% c("difference", "marginal"))) {
    stop("The revised pim.csd.m2() currently supports model='difference' or 'marginal'.")
  }
  ...
  # Restricted observed endpoints as defined in the paper:
  #   y_n = min(y_o, L), delta_n = I(y_n == L) + I(y_n < L) * delta_o
  # The first column of datap/datap_n is only a placeholder and is not used by merge.csd().
  ...
  trt_con <- data.frame(trt_con)
  design_obj <- .build_pair_design_csd(formula, data, trt_con, model, na.action)
  x <- design_obj$x
  ...
  # For the unadjusted method, this reproduces the resolved-pair indicator logic.
