...
censor_choice = function(case){
  if(case == 1){
    lambda_d = 0.15; lambda_h = 0.3; beta_d = 10; beta_h = 8
  }else if(case == 2){
    lambda_d = 0.2; lambda_h = 0.3; beta_d = 10; beta_h = 8
  }else if(case == 3){
    lambda_d = 0.15; lambda_h = 0.3; beta_d = 10; beta_h = 6
  }
  res = list(lambda_d=lambda_d, lambda_h = lambda_h, 
...
for(case in case_set){
  ...
  for(i in 1:N){
    ...
    y = 1-eval(lhs(ff), envir = penv)
    #make those ties comparison results do not contribute in score function
    x = as.numeric(x)    
    lmreg = lm(y~x-1)
    beta_est[i,case] = coef(lmreg)
  }
  cat("end_of_case_",case,"\n")
}
apply(beta_est,2,mean)
#[1] 0.6947183 0.7167179 0.6947183

#for truncated time case
###censoring rate chosen functions
censor_choice = function(case){
  if(case == 1){
